...
if __name__ == "__main__":
    ...
    parser.add_argument('--sdae_pre_lr', type=float, default=0.1, metavar='N',
                        help='learning rate for training (default: 0.001)')
    parser.add_argument('--sdae_fit_lr', type=float, default=0.1, metavar='N',
                        help='learning rate for training (default: 0.001)')
    parser.add_argument('--dcn_lr', type=float, default=0.0005, metavar='N',
                        help='learning rate for training (default: 0.001)')
    ...
    for i in range(1, repeat+1):
        ...
        if os.path.exists(sdae_savepath)==False:
            ...
            sdae.fit(train_loader, lr=args.sdae_fit_lr, num_epochs=20, corrupt=0.2, loss_type="mse")
        ...
        # if True:
            ...
            fit_train=myDataset(train_path,-1, '.data')
            ...
            dcn = DeepClusteringNetwork(input_dim=12, z_dim=8, n_centroids=3, binary=False,
                                        encodeLayer=[12], decodeLayer=[12], activation="relu",
                                        dropout=0,writer=writer,log_dir=log_dir,lambd=0.1)
            print(dcn)
            dcn.load_model(sdae_savepath)
            dcn.fit(train_loader, lr=args.dcn_lr, batch_size=batch_size, num_epochs=200)
