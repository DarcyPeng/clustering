    ...
    for param_group in optimizer.param_groups:
        if param_group["lr"]!=lr:
            ...
            if toprint:
                ...
                # write_log("Switching to learning rate %f" % lr)
                toprint = False
...
class StackedDAE(nn.Module):
    def __init__(self, input_dim=784, z_dim=10, binary=True,
        ...
        dropout=0, tied=False,log_dir='logs'):
    ...
    def pretrain(self, trainloader, lr=0.001, batch_size=128, num_epochs=10, corrupt=0.2, loss_type="cross-entropy"):
        ...
        for l in range(1, len(self.layers)):
            ...
            if l!= len(self.layers)-1:
                dae = DenoisingAutoencoder(infeatures, outfeatures, activation=self.activation, dropout=corrupt,log_dir=self.log_dir)
            else:
                dae = DenoisingAutoencoder(infeatures, outfeatures, activation="none", dropout=0,log_dir=self.log_dir)
            print(dae)
            write_log(dae,self.log_dir)
            if l==1:
                dae.fit(trloader, lr=lr, batch_size=batch_size, num_epochs=num_epochs, corrupt=corrupt, loss_type=loss_type)
            else:
                if self.activation=="sigmoid":
                    dae.fit(trloader, lr=lr, batch_size=batch_size, num_epochs=num_epochs, corrupt=corrupt, loss_type="cross-entropy")
                else:
                    dae.fit(trloader, lr=lr, batch_size=batch_size, num_epochs=num_epochs, corrupt=corrupt, loss_type="mse")
    ...
    def fit(self, trainloader, lr=0.001, num_epochs=10, corrupt=0.3,
        ...
        write_log("=====Stacked Denoising Autoencoding Layer=======",self.log_dir)
        ...
        for epoch in range(num_epochs):
            ...
            #     epoch+1, train_loss / len(trainloader.dataset), valid_loss / len(validloader.dataset)),self.log_dir)

            print("#Epoch %3d: Reconstruct Loss: %.4f" % (
                epoch + 1, train_loss / len(trainloader.dataset)))
            write_log("#Epoch %3d: Reconstruct Loss: %.4f" % (
                epoch + 1, train_loss / len(trainloader.dataset)), self.log_dir)
