...
import numpy as np
import math
from lib.utils import Dataset, masking_noise,write_log
    ...
    for param_group in optimizer.param_groups:
        if param_group["lr"]!=lr:
            ...
            if toprint:
                ...
                # write_log("Switching to learning rate %f" % lr,log_dir)
                toprint = False

class DenoisingAutoencoder(nn.Module):
    def __init__(self, in_features, out_features, activation="relu", 
        dropout=0.2, tied=False,log_dir='logs'):
    ...
    def fit(self, trainloader, lr=0.001, batch_size=128, num_epochs=10, corrupt=0.3,
        loss_type="mse"):
        """
        data_x: FloatTensor
        valid_x: FloatTensor
